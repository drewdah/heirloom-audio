...
// VU meter constants
const METER_BARS = 36;
const METER_CLIP_FRAC = 0.92;
const METER_WARN_FRAC = 0.75;
...
export default function AudioSettingsModal({ open, onClose }: AudioSettingsModalProps) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed z-50 top-0 left-1/2 -translate-x-1/2 w-full"
        style={{ maxWidth: 420, paddingTop: "10vh" }}>
        <div
          ...
          <div className="flex items-center justify-between px-5 pt-5 pb-4"
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: "rgba(107,21,21,0.15)", border: "1px solid rgba(107,21,21,0.3)" }}>
              ...
              <div className="flex items-center gap-2">
                {[0.75, 1.0, 1.5, 2.0, 2.5].map(v => (
                  <button
                    ...
                    style={{
                    fontSize: "0.8rem",
                      background: Math.abs(micGain - v) < 0.01 ? "rgba(107,21,21,0.25)" : "rgba(255,255,255,0.05)",
                      color: Math.abs(micGain - v) < 0.01 ? "var(--accent)" : "var(--text-tertiary)",
                      border: Math.abs(micGain - v) < 0.01 ? "1px solid rgba(107,21,21,0.4)" : "1px solid transparent",
...
function drawMeter(
  ...
  // Use dB scaling: map -60dBFS..0dBFS → 0..1 so normal speech (~-20dBFS) lands ~65% across
  const toLevel = (v: number) => v > 0 ? Math.max(0, Math.min(1, (20 * Math.log10(v) + 60) / 60)) : 0;
  const activeCount = Math.round(toLevel(rms) * METER_BARS);
  const peakBar = Math.min(Math.round(toLevel(peak) * (METER_BARS - 1)), METER_BARS - 1);
  ...
  for (let i = 0; i < METER_BARS; i++) {
    ...
    if (active || isPeak) {
      ...
      if (frac >= METER_CLIP_FRAC) ctx.fillStyle = "rgba(255,59,48,0.15)";
      else if (frac >= METER_WARN_FRAC) ctx.fillStyle = "rgba(255,214,10,0.12)";
